...
class SpanWithinTest extends BaseTest
{
    ...
    public function testToArray()
    {
        ...
        $spanContainingQuery = new SpanWithin($spanTermQuery3, $spanNearQuery);
    ...
    public function testSpanWithin()
    {
        ...
        $spanContainingQuery = new SpanWithin(new SpanTerm([$field => 'amet']), $spanNearQuery);
        $resultSet = $type->search($spanContainingQuery);
        $this->assertEquals(1, $resultSet->count());

        $spanContainingQuery = new SpanWithin(new SpanTerm([$field => 'not-matching']), $spanNearQuery);
        $resultSet = $type->search($spanContainingQuery);
